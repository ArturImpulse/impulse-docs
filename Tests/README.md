...
## 🔍 Принцип работы конфигурации базы данных

### 📋 Алгоритм работы функции `getenv()`

Функция `getenv()` работает по следующему алгоритму:

* **Основной процесс**: пытается получить значение переменной окружения `TEST_DB_NAME` из файла test_db.php который лежит в config

* **Условия обработки**:
  * Если переменная **не определена** в окружении
  * Или если переменная **определена, но пуста**

  В этих случаях используется `'значение по умолчанию'`

### 🎯 Важные особенности

* **Правило применения**: значение по умолчанию активируется автоматически при недоступности переменной окружения

* **Приоритетная последовательность**:
  1. Значение из окружения (файл `.env`)
  2. Значение по умолчанию (указанное в функции)

### ⚠️ Рекомендации

* Всегда проверяйте корректность загрузки файла `.env`
* Убедитесь в правильности указания переменных окружения
* Проверяйте права доступа к конфигурационным файлам
* Следите за актуальностью значений в `.env` файле
---
